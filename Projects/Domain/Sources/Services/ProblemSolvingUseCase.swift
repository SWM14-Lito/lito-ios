    ...
    public func getProblemInfo() -> AnyPublisher<ProblemDetailVO, Error> {
        ...
        Future<ProblemDetailVO, Error> { promise in
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                let vo = ProblemDetailVO(question: "문맥 전환 (Context Switching)이 무엇인가?", answer: "CPU가 이전 상태의 프로세스를 PCB에 보관하고, 또 다른 프로세스를 PCB에서 읽어 레지스터에 적재하는 과정.", keyword: "PCB", favorite: .favorite)
                promise(.success(vo))
            }
        }
        .eraseToAnyPublisher()
